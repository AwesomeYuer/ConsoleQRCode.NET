...
namespace Cyjb.Reflection
{
	...
	/// <para>关于 <see cref="Binder"/> 类的原理，以及 <see cref="PowerBinder"/> 类的实现，可以参见我的博文
	...
	public sealed class PowerBinder : Binder
	{
		...
		public static PowerBinder Default
		{
			get
			{
				return defaultInstance ??= new PowerBinder(false);
			}
		}
		...
		public static PowerBinder Explicit
		{
			get
			{
				return explicitInstance ??= new PowerBinder(true);
			}
		}
		...
		/// <param name="culture">一个 <see cref="CultureInfo"/> 实例，用于控制数据类型的强制转换。</param>
		...
		public override object ChangeType(object value, Type type, CultureInfo? culture)
		{
			...
			{
				return GenericConvert.ChangeType(value, type)!;
			}
		...
		/// <param name="culture">一个 <see cref="CultureInfo"/> 实例，用于在强制类型的联编程序实现中控制数据类型强制。</param>
		...
		public override FieldInfo BindToField(BindingFlags bindingAttr, FieldInfo[] match, object value, CultureInfo? culture)
		{
			...
			if (bindingAttr.HasFlag(BindingFlags.SetField))
			{
				...
				if (length == 0)
				{
					throw ReflectionExceptions.MissingField();
				}
				if (length > 1 && valueType != null)
				{
					// 多个可匹配字段，尝试寻找类型最匹配的字段。
					length = FilterMember(match, length,
						(firstField, secondField) => CompareType(firstField.FieldType, secondField.FieldType, valueType));
				}
			}
			else
			{
			...
			if (best == null)
			{
				throw ReflectionExceptions.AmbiguousMatchField();
			}
			return best;
		}
		...
		private int CompareType(Type firstType, Type secondType, Type? type)
		{
			...
			ConversionType convType = ConversionFactory.GetStandardConversion(firstType, secondType);
			...
			ParameterModifier[]? modifiers)
		{
			...
			if (bindingAttr.HasFlag(BindingFlagsUtil.VarArgsParamBinding))
			{
				options |= MethodArgumentsOption.VarArgsParamBinding;
			}
		...
		private int CompareMethod(MethodMatchInfo firstMethod, MethodMatchInfo secondMethod, Type?[] types)
		{
			...
			// 不展开可变参数的方法更为匹配
			if (firstMethod.HasOptionalArgument)
			{
				if (secondMethod.HasOptionalArgument)
				{
					// 可变参数个数少的更为匹配。
					int len1 = firstMethod.OptionalArgumentTypes?.Count ?? 0;
					int len2 = secondMethod.OptionalArgumentTypes?.Count ?? 0;
					if (len1 != len2)
					{
						return len1 - len2;
					}
			...
			else if (secondMethod.HasOptionalArgument)
			{
				return -1;
			}
		...
		private class MethodMatchInfo
		{
				...
				int[]? paramOrder, MethodArgumentsOption options)
			{
				...
				if (method.IsGenericMethodDefinition)
				{
					...
					Type[]? genericArguments = argsInfo.GetGenericArguments(null, options);
				...
				{
					// 由于设置了 INVOCATION_FLAGS.INVOCATION_FLAGS_NO_INVOKE
					// RuntimeMethodInfo.Invoke 不支持调用可变参数的方法。
					if (!options.HasFlag(MethodArgumentsOption.VarArgsParamBinding))
					{
						return null;
					}
				}
			...
			/// 方法的可选参数的类型。
			/// </summary>
			public readonly IList<Type>? OptionalArgumentTypes;
			/// <summary>
			/// 方法是否包含了可选参数。
			/// </summary>
			public readonly bool HasOptionalArgument;
			/// <summary>
			...
			private MethodMatchInfo(MethodBase method, MethodArgumentsInfo argsInfo, int[]? paramOrder)
			{
				...
				// argsInfo 的固定参数可能长度不足，且可能包含泛型参数，因此这里需要重新提取一下固定参数。
				if (argsInfo.ParamArrayType != null)
				{
					FixedParameters = method.GetParametersNoCopy()[0..^1];
					ParamArrayType = method.GetParametersNoCopy()[^1].ParameterType;
					ParamArrayElementType = ParamArrayType.GetElementType();
				}
				else if (argsInfo.OptionalArgumentTypes != null)
				{
					FixedParameters = method.GetParametersNoCopy();
					OptionalArgumentTypes = argsInfo.OptionalArgumentTypes!;
				}
				else
				{
					FixedParameters = method.GetParametersNoCopy();
				}
				HasOptionalArgument = method.CallingConvention.HasFlag(CallingConventions.VarArgs);
			...
			public Type GetParameterType(int index)
			{
				...
				{
					return OptionalArgumentTypes![index - FixedParameters.Length];
				}
			}
		}
			...
			ParameterModifier[]? modifiers, CultureInfo? culture, string[]? names, out object? state)
		{
			...
			{
				throw ReflectionExceptions.DuplicateName(nameof(names));
			}
			MethodArgumentsOption options = MethodArgumentsOption.None;
			if (isExplicit)
			{
			...
			if (length == 0)
			{
				// 没有可匹配的方法。
				state = null;
				throw ReflectionExceptions.MissingMethod();
			}
			info = SelectMethod(infos, length, types);
			if (info == null)
			{
				state = null;
				throw ReflectionExceptions.AmbiguousMatchMethod();
			}
		...
		private static void UpdateArgs(MethodMatchInfo match, ref object?[] args, out object? state)
		{
			...
			else if (match.OptionalArgumentTypes != null)
			{
				length += match.OptionalArgumentTypes.Count;
			}
			...
			for (int i = 0; i < fixedLen; i++)
			{
				...
				{
					newArgs[i] = ArrayUtil.Empty(param.ParameterType.GetElementType()!);
				}
				else
				{
			...
			Type? returnType, Type[]? indexes, ParameterModifier[]? modifiers)
		{
			...
			if (best == null)
			{
				throw ReflectionExceptions.AmbiguousMatchProperty();
			}
			return best;
		}
