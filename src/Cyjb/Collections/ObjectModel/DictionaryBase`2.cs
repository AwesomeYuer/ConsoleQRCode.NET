	...
	IReadOnlyDictionary<TKey, TValue>, IDictionary, ICollection<KeyValuePair<TKey, TValue>>
{
	...
	/// <exception cref="ArgumentNullException"><paramref name="key"/> 为 <c>null</c>。</exception>
	/// <exception cref="ArgumentException">当前字典中已存在具有相同键的元素。</exception>
	...
	object? IDictionary.this[object key]
	{
		...
		set
		{
			...
			try
			{
				...
				{
					CommonExceptions.ArgumentWrongType(value, typeof(TValue));
				}
			}
			catch (InvalidCastException)
			{
				CommonExceptions.ArgumentWrongType(key, typeof(TKey));
			}
		}
	}
	...
	/// <exception cref="ArgumentNullException"><paramref name="keyValuePair"/> 的键为 <c>null</c>。</exception>
	/// <exception cref="ArgumentException">当前字典中已存在具有相同键的元素。</exception>
